...
word *Abc_NtkVerifySimulatePatternParallel(Abc_Ntk_t *pNtk, word *pModel)
{
    ...
    // Set the CI values
    Abc_AigConst1(pNtk)->pCopy = (Abc_Obj_t *)(ABC_PTRINT_T)0xFFFFFFFFFFFFFFFFULL; // Assuming constant 1 is all 1s
    Abc_NtkForEachCi(pNtk, pNode, i)
        pNode->pCopy = (Abc_Obj_t *)(ABC_PTRINT_T)pModel[i];
    ...
    Abc_NtkForEachNode(pNtk, pNode, i)
    {
        Value0 = ((uint64_t)(ABC_PTRINT_T)Abc_ObjFanin0(pNode)->pCopy) ^ ((uint64_t)Abc_ObjFaninC0(pNode) ? 0xFFFFFFFFFFFFFFFFULL : 0x0);
        Value1 = ((uint64_t)(ABC_PTRINT_T)Abc_ObjFanin1(pNode)->pCopy) ^ ((uint64_t)Abc_ObjFaninC1(pNode) ? 0xFFFFFFFFFFFFFFFFULL : 0x0);
        pNode->pCopy = (Abc_Obj_t *)(ABC_PTRINT_T)(Value0 & Value1);
    }

    // Fill the output values
    pValues = ABC_ALLOC(uint64_t, Abc_NtkCoNum(pNtk));
    Abc_NtkForEachCo(pNtk, pNode, i)
    {
        pValues[i] = ((uint64_t)(ABC_PTRINT_T)Abc_ObjFanin0(pNode)->pCopy) ^ ((uint64_t)Abc_ObjFaninC0(pNode) ? 0xFFFFFFFFFFFFFFFFULL : 0x0);
    }
