...
std::vector<std::pair<int, int>> ComputeOdc(Abc_Ntk_t* pNtk, Abc_Obj_t* pNode) {
    ...
    // Find the fanout node (node that consumes our target node's output)
    ...
    if (Abc_ObjFanin0(pFanout) == pNode) {
        ...
        isFaninComplemented = Abc_ObjFaninC1(pFanout);
    } else {
        pOtherFanin = Abc_ObjFanin0(pFanout);
        isFaninComplemented = Abc_ObjFaninC0(pFanout);
    }
    
    // Create cone for the other fanin
    ...
    // Map primary inputs to variables
    Aig_Obj_t* pObj;
    int i;
    Aig_ManForEachCi(pAig, pObj, i) {
        nodeToVar[Aig_ObjId(pObj)] = varCounter++;
    ...
    // Add output constraint for controlling value
    Aig_ManForEachCo(pAig, pObj, i) {
        int driverId = Aig_ObjFaninId0(pObj);
        int isCompl = Aig_ObjFaninC0(pObj);
        int outVar = nodeToVar[driverId];

        printf("DriverID: %d, isCompl: %d, outVar: %d\n", driverId, isCompl, outVar);
        
        // Add clause to force the output to controlling value (considering edge complement)
        lit Lits[1];
        Lits[0] = toLitCond(outVar, !(0 ^ isCompl ^ isFaninComplemented));
        ...
        break;  // We only need one constraint
    }
    ...
    while (sat_solver_solve(pSat, NULL, NULL, 0, 0, 0, 0) == l_True) {
        int val0 = sat_solver_var_value(pSat, 1);
        int val1 = sat_solver_var_value(pSat, 2);

        printf("Max id: %d\n", Aig_ManObjNumMax(pAig));
        
        // Account for edge complementation to our target node
        if (Abc_ObjFanin0(pFanout) == pNode) {
            val0 = val0 ^ Abc_ObjFaninC0(pFanout);
            val1 = val1 ^ Abc_ObjFaninC0(pFanout);
        } else {
            val0 = val0 ^ Abc_ObjFaninC1(pFanout);
            val1 = val1 ^ Abc_ObjFaninC1(pFanout);
        }
        
        printf("Found pattern: %d%d\n", val0, val1);
        careSet.insert({val0, val1});
        
        // Block this solution
        lit blockLits[2];
        blockLits[0] = toLitCond(1, val0);
        blockLits[1] = toLitCond(2, val1);
        if (!sat_solver_addclause(pSat, blockLits, blockLits + 2)) break;
    }
    
    // Clean up SAT solver resources
    sat_solver_delete(pSat);
    Aig_ManStop(pAig);
    
    // Compute final ODC patterns
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            if (!isPatternPossible(pNode, i, j)) {
                printf("SDC found: %d%d\n", i, j);
                continue;
            }
            if (careSet.find({i, j}) == careSet.end()) {
                printf("ODC found: %d%d\n", i, j);
                odcPatterns.push_back({i, j});
            }
        }
    }
