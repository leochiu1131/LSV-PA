...
OdcResult calculateOdc(Abc_Ntk_t* pNtk, Abc_Obj_t* pNode) {
    ...
    while (true) {
        ...
        if (status == l_False)
        {
            printf("UNSAT the first time");
            break;
        }
        
        if (status == l_True) {
            // Map original fanin IDs to AIG IDs
            int aigFanin0Id = GetAigNodeId(pMapping, fanin0Id);
            int aigFanin1Id = GetAigNodeId(pMapping, fanin1Id);
            
            // Get corresponding SAT variables
            int var0 = nodeToVar[aigFanin0Id];
            int var1 = nodeToVar[aigFanin1Id];
            
            // Debug output
            printf("Original fanin IDs: %d, %d\n", fanin0Id, fanin1Id);
            printf("Mapped AIG IDs: %d, %d\n", aigFanin0Id, aigFanin1Id);
            printf("SAT variables: %d, %d\n", var0, var1);
            
            if (var0 == 0 || var1 == 0) {
                printf("Warning: Failed to map node to SAT variable\n");
                continue;
            }
            ...
            // Account for complemented edges
            if (isCompl0) val0 = !val0;
            if (isCompl1) val1 = !val1;
            
            std::pair<int, int> pattern = {val0, val1};
    ...
    // Step 8: Output found ODC patterns
    if (result.odcPatterns.empty()) {
        printf("No ODC patterns found\n");
    } else {
        for (const auto& pattern : result.odcPatterns) {
